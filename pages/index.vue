...
export default {
  ...
  computed: {
    options() {
      const optionlist = [];
      this.shopData.forEach(element => optionlist.push(element.cityName));
      return optionlist;
    }
    // optionsを作る
    // options() {
    //   const list = [];
    //   for (let index = 0; index < this.shopData.data.result.length; index++) {
    //     console.log(index);
    //     list.append({
    //       value: this.shopData.data.result[index].cityCode,
    //       text: this.shopData.data.result[index].cityName
    //     });
    //   }
    //   console.log(list);
    //   return list;
  ...
  mounted() {
    axios
      .get("https://opendata.resas-portal.go.jp/api/v1/cities?prefCode=30", {
        headers: { "X-API-KEY": "dfVSnH3g38Rp6Rl8WuAsvWvioJO2X8vT0K6mYHf4" }
      })
      .then(response => (this.shopData = response.data.result));
